...
import { AnalyticsService, AnalyticsEventInputSchema } from "./services/analytics";
...
const app = new Elysia()
  ...
  .use(cors({
    ...
    allowedHeaders: ['Content-Type', 'User-Agent', 'X-Forwarded-For', 'X-Real-IP', 'CF-Connecting-IP'],
  }))
  ...
  .post('/api/event', async ({ body, headers, set }) => {
    try {
    ...
    body: t.Object({
      ...
      customProperties: t.Optional(t.Record(t.String(), t.Unknown(), { description: 'Custom event properties' })),
    }),
    tags: ['Analytics'],
    summary: 'Track page view or custom event',
    description: 'Accepts analytics events, generates privacy-safe visitor hash, and emits to Flowcore',
  })
  ...
  .get('/health', () => {
    ...
    description: 'Returns detailed service health including salt rotation and Flowcore configuration',
  })
  ...
  .post('/api/transformer', async ({ body, headers, set }) => {
    try {
